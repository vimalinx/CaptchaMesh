...
通知，否则系统可能停止后台等待。
Agent 后台接题服务只在接题开启时持有有超时并定期续期的局部 WakeLock；点击“暂停接题”或
停止服务会立即释放。荣耀等系统仍可能直接冻结前台服务；此时 App 会明确显示后台保护未开启。
在“设置”页点击“开启后台保护”后，由 Android 系统弹窗请求用户确认，App 不会静默修改该设置。
确认后回到 App，非荣耀设备状态应变为“系统后台保护已开启”，后台连接会随页面恢复重新启动。
荣耀设备还会显示“打开荣耀管家”：进入“应用启动管理”后找到 CaptchaMesh，关闭自动管理，
并允许自启动、关联启动和后台活动。Android 标准豁免与荣耀启动管理是两层独立设置，必须通过一次真实后台
到题测试确认，App 不会只根据开关存在就报告通过。
...
App 会把未捕获崩溃、后台连接失败、工作流失败、配对失败和 Agent 任务失败记录在 Android
应用私有目录中，并使用 Android Keystore 加密。记录最多保留约 16,000 个字符，较新的事件在前。

1. 打开 App 的“记录”页。
2. 点击“复制诊断”，看到“脱敏诊断已复制”。
3. 将内容粘贴到私有 Issue 或发给维护者；先保留首部的 App、Android 和设备版本。
4. 问题处理完后可点击“清空记录”，同时删除活动记录和诊断记录。

诊断只包含固定事件名、异常类名和 `app.captchamesh.*` 栈帧。它不会读取或复制异常消息、
Key、Token、Cookie、网址、二维码、挑战正文、答案或任务 ID。若需要系统级冻结证据，另行提供
经过检查的 `adb logcat` 片段；不要直接公开完整 logcat。
