...
1. **Agent API**：不启动任何手机工作流，直接向 `http://127.0.0.1:8893` 创建任务；手机应自动提醒。
2. **手机工作流**：在“工作流”页启动一个白名单脚本，由脚本提交任务；手机应显示同一人工界面。
...
- 在设置中关闭“CAPTCHA 到达时显示提醒”后不弹高优先级通知，但打开 App 仍能处理已到达任务；
