...
echo "[4/7] Installed CLI smoke test"
...
echo "[5/7] Android tests, lint and APK"
(
  cd app-src
  ./gradlew --no-daemon :app:testDebugUnitTest :app:lintDebug :app:assembleDebug
)

echo "[6/7] Release boundary recheck"
"$python_bin" tools/verify_public_release.py "$build_dir"

echo "[7/7] Git history secret scan"
